        ...
        <div className="max-w-[1400px] mx-auto px-8">
          ...
          <div className="grid gap-8 md:grid-cols-2 lg:grid-cols-3">
            {loadingProjects
                ...
                <article key={proj._id} className="group bg-white border border-gray-200 rounded-2xl overflow-hidden shadow-md hover:shadow-2xl hover:border-primary-500 hover:-translate-y-1 transition-all duration-300">
                  ...
                  <div className="p-6 pb-0">
                    <div className="flex gap-2 flex-wrap mb-3">
                      {proj.tags.map((tag, j) => (
                        <span key={j} className={`px-3 py-1.5 rounded-lg text-xs font-bold tracking-wider uppercase border ${DIFFICULTY_COLORS[tag] || 'text-blue-700 border-blue-200 bg-blue-50'}`}>{tag}</span>
                      ))}
                    </div>
              ...
              : (
                // Fallback: show static placeholder projects while DB is being seeded
                [
        ...
        <div className="max-w-[1400px] mx-auto px-8">
          ...
          <div className="grid gap-8 md:grid-cols-2 lg:grid-cols-3">
            {loadingTutorials
              ...
              : tutorials.length > 0
              ? tutorials.map((tut) => (
                <Link href={`/tutorials/${tut.slug}`} key={tut._id} className="group bg-white border border-gray-200 rounded-2xl overflow-hidden shadow-md hover:shadow-2xl hover:border-primary-500 hover:-translate-y-1 transition-all duration-300 no-underline">
                  {tut.imageURL && (
                  ...
                  <div className="p-5">
                    <span className={`inline-block px-3 py-1 rounded-lg text-xs font-bold tracking-wider uppercase border mb-3 ${DIFFICULTY_COLORS[tut.difficulty] || ''}`}>{tut.difficulty}</span>
                    ...
                    <p className="text-gray-600 text-sm line-clamp-2 mb-3 leading-relaxed">{tut.description}</p>
              ...
              : (
                [
                  ...
                  <Link href={`/tutorials/${tut.slug}`} key={tut.id} className="group bg-white border border-gray-200 rounded-2xl overflow-hidden shadow-md hover:shadow-2xl hover:border-primary-500 hover:-translate-y-1 transition-all duration-300 no-underline">
                    <div className="h-[160px] overflow-hidden relative">
                    ...
                    <div className="p-5">
                      <span className={`inline-block px-3 py-1 rounded-lg text-xs font-bold tracking-wider uppercase border mb-3 ${DIFFICULTY_COLORS[tut.level] || ''}`}>{tut.level}</span>
                      <h3 className="font-bold text-base text-gray-900 mb-2 group-hover:text-primary-600 transition-colors line-clamp-2">{tut.title}</h3>
            ...
            {MCU_PLATFORMS.map((mcu, i) => (
              <div key={i} className="group bg-white border border-gray-200 rounded-2xl overflow-hidden shadow-md hover:shadow-2xl hover:border-primary-500 hover:-translate-y-1 transition-all duration-300">
                ...
                <div className="p-6">
                  ...
                  <Link href="/microcontrollers" className="inline-block px-8 py-4 bg-[#1e3a8a] text-white rounded-xl text-sm font-bold hover:bg-[#1e40af] transition-all shadow-lg hover:shadow-xl transform hover:-translate-y-1">
                    Explore {mcu.name} →
                  </Link>
                </div>
              </div>
